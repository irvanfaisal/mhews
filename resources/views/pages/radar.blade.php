            ...
            <div id="map" class="min-h-screen">
                <div id="menu-title">
                    ...
                    <div class="my-auto mt-2 pt-1 border-t">
                        <p id="map-title" class="text-white font-bold text-xl uppercase" style="text-shadow: 0 0 10px rgba(0, 0, 0,1);">Pengamatan Radar di Indonesia</p>
                        <p id="map-date" class="text-white" style="text-shadow: 0 0 10px rgba(0, 0, 0,1);">-</p>
                    </div>
                </div>

                <div id="menu-layer">
                    <input id="date" type='date' class="bg-paleblue-light bg-opacity-75 p-1 m-1 rounded-sm" required value="{{ \Carbon\Carbon::now()->format('Y-m-d') }}" onchange="getRadar();">
                    <select id="time" class="bg-paleblue-light bg-opacity-75 p-1 m-1 rounded-sm" onfocus='this.size=10;' onblur='this.size=1;' onchange='this.size=1; this.blur();getRadar();'>
                        @for($i = 0;$i<24;$i++)                        
...
<script type="text/javascript">
    ...
    document.getElementById("menu-radar").classList.add("bg-white");
    document.getElementById("menu-radar").childNodes[1].childNodes[1].classList.remove("text-white");
    document.getElementById("menu-radar").childNodes[1].childNodes[1].classList.add("text-orange-500");
    document.getElementById("menu-radar").childNodes[1].childNodes[3].classList.remove("text-white");
    document.getElementById("menu-radar").childNodes[1].childNodes[3].classList.add("text-orange-500");
</script>
@endsection