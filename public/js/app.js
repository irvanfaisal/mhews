...
window.initMap = function () {
  ...
  map = L.map('map', {
    ...
    maxBounds: bounds,
    minZoom: 5
  }).setView([-1.7081, 118.797175], 5); // var CartoDB_DarkMatterNoLabels  = L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}{r}.png', {
  //        maxZoom: 20
  //    }).addTo(map);
  ...
  }).addTo(map); // let layerStatic = L.esri.dynamicMapLayer({
  ...
  omnivore.topojson(flagsUrl + 'data/provinsi.json', null, province).on('ready', function () {}).setZIndex(-999).bringToBack().addTo(map);
...
window.getEarthquake = function () {
  ...
  var tableEarthquake = document.getElementsByClassName('table-earthquake');

  for (var i = 0; i < tableEarthquake.length; i++) {
    tableEarthquake[i].classList.add("hidden");
  }

  var sources = document.getElementsByName("source-earthquake");
  var dataSource;
  ...
  for (var _i = 0; _i < tab.length; _i++) {
    ...
    tab[_i].classList.remove("bg-blue-500");
  }
  ...
  for (var i = 0; i < timesteps.length; i++) {
    if (timesteps[i].checked) {
      var timestep = timesteps[i].value;

      if (timestep == "month") {
        tab[2].classList.add('active');
        tab[2].classList.add('bg-blue-500');
        var classString = tab[2].getAttribute('data-target');
      ...
      if (timestep == "week") {
        ...
        var _classString = tab[1].getAttribute('data-target');

        document.getElementById('panels').getElementsByClassName(_classString)[0].classList.add("active");
      ...
      if (timestep == "day") {
        ...
        var _classString2 = tab[0].getAttribute('data-target');

        document.getElementById('panels').getElementsByClassName(_classString2)[0].classList.add("active");
  ...
  if (dataSource == "bmkg") {
    ...
    var table = document.getElementsByClassName('table-bmkg');

    for (var i = 0; i < table.length; i++) {
      table[i].classList.remove("hidden");
    }

    for (var _i3 = 0; _i3 < data.bmkg.length; _i3++) {
      if (moment(data.bmkg[_i3].waktu) >= date) {
        var radius = void 0;

        var _color = void 0;

        var _colorAnimation = void 0;

        if (parseFloat(data.bmkg[_i3].Magnitude) > 4 && parseFloat(data.bmkg[_i3].Magnitude) <= 5) {
          level = 2;
          _color = "#37b896";
          _colorAnimation = "green";
        } else if (parseFloat(data.bmkg[_i3].Magnitude) > 5 && parseFloat(data.bmkg[_i3].Magnitude) <= 6) {
          level = 3;
          _color = "#ffc107";
          _colorAnimation = "yellow";
        } else if (parseFloat(data.bmkg[_i3].Magnitude) > 6 && parseFloat(data.bmkg[_i3].Magnitude) <= 7) {
          level = 4;
          _color = "#dda15a";
          _colorAnimation = "orange";
        } else if (parseFloat(data.bmkg[_i3].Magnitude) > 7) {
          level = 5;
          _color = "#e02f22";
          _colorAnimation = "red";
        } else {
          level = 1;
          _color = "#4bb2e8";
          _colorAnimation = "light-blue";
        }

        var randomPulse = Math.floor(Math.random() * 5 + 1);
        var myIcon_pulse = L.divIcon({
          className: 'css-icon',
        ...
        var coordinates = data.bmkg[_i3].point.coordinates.split(',');

        var point = L.marker([coordinates[0], coordinates[1]], {
          icon: myIcon_pulse
        });
        var customOptions = {
          'className': 'custom-width'
        };
        url = "https://data.bmkg.go.id/DataMKG/TEWS/" + moment(data.bmkg[_i3].DateTime).format('YYYYMMDDHHmmss') + ".mmi.jpg";
        point.bindPopup('<div class="custom-scrollbar flex" style="width:390px;max-height:200px;overflow-y:auto;"><img onclick="showModal(this)" style="max-width:150px;" src="https://data.bmkg.go.id/DataMKG/TEWS/' + moment(data.bmkg[_i3].DateTime).format('YYYYMMDDHHmmss') + '.mmi.jpg"><div><p class="my-1 p-1 uppercase font-bold bg-animation-red">' + data.bmkg[_i3].Keterangan + '</p><p class="my-1 p-1"><span class="text-blue-light font-bold">Magnitude:</span> ' + data.bmkg[_i3].Magnitude + '</p><p class="my-1 p-1 bg-white bg-opacity-20"><span class="text-blue-light font-bold">Kedalaman:</span> ' + data.bmkg[_i3].Kedalaman + '</p><p class="my-1 p-1"><span class="text-blue-light font-bold">Lokasi:</span> ' + data.bmkg[_i3].Wilayah + '</p><p class="my-1 p-1 my-1 p-1 bg-white bg-opacity-20"><span class="text-blue-light font-bold">Waktu:</span> ' + moment(data.bmkg[_i3].waktu).format('DD MMMM YYYY HH:mm:ss') + ' WIB</p><p class="my-1 p-1 text-xs"><span class="text-blue-light font-bold">Sumber:</span> BMKG</p></div></div>', customOptions);
  ...
  if (dataSource == 'usgs') {
    ...
    var _table = document.getElementsByClassName('table-usgs');

    for (var i = 0; i < _table.length; i++) {
      _table[i].classList.remove("hidden");
    }

    earthquakeLayer = L.geoJson(null, {
      filter: function filter(geoJsonFeature) {
  ...
  setTimeout(function () {
    document.getElementById("loader").classList.remove("show-animation");
  }, 250); // omnivore.geojson(data, null, earthquakeLayer).addTo(map);  
};

window.showModal = function (url) {
  var modal = document.getElementById("myModal");
  modal.style.display = "block"; // Get the image and insert it inside the modal - use its "alt" text as a caption

  var modalImg = document.getElementById("img01");
  modalImg.src = url.getAttribute("src"); // Get the <span> element that closes the modal
...
window.getLatestEarthquake = function () {
  ...
  var coordinates = data.Infogempa.gempa.Coordinates.split(',');
...
window.getObservationStations = function () {
  ...
  var _loop = function _loop(_i4) {
    ...
    if (stations[_i4].last) {
      if (moment(stations[_i4].last) >= now) {
        color = '#43a047';
      } else if (moment(stations[_i4].last) >= week) {
        color = '#eab308';
      } else {
        color = '#7f1d1d';
      }
    ...
    var point = L.circleMarker([stations[_i4].lat, stations[_i4].lon], {
      radius: 5,
      fillColor: color,
      color: "#000",
      weight: 0.25,
    ...
    point.bindPopup('<div><img class="my-1 p-1" src="https://tangkalbencana.id/img/station/' + stations[_i4].station_id + '_bangunan.jpg" style="max-height:80px;"><p class="my-1 p-1 bg-white bg-opacity-20"><span class="text-blue-400 font-bold">ID:</span> ' + stations[_i4].station_id + '</p><p class="my-1 p-1"><span class="text-blue-400 font-bold">Nama:</span> ' + stations[_i4].name + '</p><p class="my-1 p-1 bg-white bg-opacity-20"><span class="text-blue-400 font-bold">Data Terakhir:</span> ' + (stations[_i4].last ? moment(stations[_i4].last).format('YYYY-MM-DD HH:mm') : '-') + '</p>' + (stations[_i4].last ? '<button class="px-2 py-1 my-1 font-xs bg-yellow-600" onclick="getLastObservation(' + moment(stations[_i4].last) + ')">Lihat Data Terakhir</button>' : '') + '</div>', customOptions);
    stationLists.push(point);
    point.on('click', function (e) {
      document.querySelector('#station [value="' + stations[_i4].station_id + '"]').selected = true;
    });
  };

  for (var _i4 = 0; _i4 < stations.length; _i4++) {
    var customOptions;
...
window.getDetailForecast = function (date) {
  ...
  fetch('getDetailForecast?' + new URLSearchParams(data), {
    ...
    content = ""; // document.getElementById("modal-title").innerHTML = "Prediksi Cuaca " + moment(date).format("D MMMM YYYY");
    ...
    for (i = 0; i < data.forecast.length; i++) {
      ...
      content += "<tr class='border-b border-opacity-25 border-blue-500'>" + "<td class='text-center px-2 py-1'>" + moment(data.forecast[i].date).format("HH:mm") + "</td>" + "<td class='text-center px-2 py-1'>" + parseFloat(data.forecast[i].temperature.toFixed(1)).toLocaleString("id") + "<sup>o</sup>C</td>" + "<td class='text-center px-2 py-1'>" + (parseFloat(data.forecast[i].rain) > 0 ? parseFloat(parseFloat(data.forecast[i].rain).toFixed(1)).toLocaleString("id") + ' mm/jam' : '-') + "</td>" + "<td class='text-center px-2 py-1'>" + (parseFloat(data.forecast[i].rain) < 1 ? "Cerah" : parseFloat(data.forecast[i].rain) < 5 ? "Hujan Ringan" : parseFloat(data.forecast[i].rain) < 10 ? "Hujan Sedang" : parseFloat(data.forecast[i].rain) < 20 ? "Hujan Lebat" : "Hujan Sangat Lebat") + "</td>" + "</tr>";
    }

    document.getElementById("table-daily-content").innerHTML = content;
    ;
    document.getElementById("daily-title").innerHTML = "Prediksi Cuaca " + data.regency;
    document.getElementById("daily-date").innerHTML = moment(data.date).format("D MMMM YYYY"); // document.getElementById("daily-duration").innerHTML = count + ' Jam';
...
window.getExtremeWeather = function () {
  ...
  function groupBy(xs, f) {
    return xs.reduce(function (r, v, i, a) {
      var k = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : f(v);
  ...
  if (checked) {
    ...
    fetch(dataUrl + '/area-' + moment(currentDate).format("YYYYMMDD") + '.json', {
      ...
      fetch(dataUrl + '/' + moment(currentDate).format("YYYYMMDD") + '.json', {
        ...
        dataList = dataList.filter(function (i) {
          return filterLists.includes(parseInt(i.category));
        });
        feature_group = groupBy(dataList, function (c) {
          return c.feature_id;
        });

        for (var _i5 = 0, _Object$entries = Object.entries(feature_group); _i5 < _Object$entries.length; _i5++) {
          var _Object$entries$_i = _slicedToArray(_Object$entries[_i5], 2),
              key = _Object$entries$_i[0],
              value = _Object$entries$_i[1];

          tmp = groupBy(value, function (c) {
            return c.PROVINSI;
          });
          dataTmp[key] = []; // console.log(tmp)

          for (var _i6 = 0, _Object$entries2 = Object.entries(tmp); _i6 < _Object$entries2.length; _i6++) {
            var _Object$entries2$_i = _slicedToArray(_Object$entries2[_i6], 2),
                key2 = _Object$entries2$_i[0],
                value2 = _Object$entries2$_i[1];

            dataTmp[key][key2] = _toConsumableArray(new Set(value.map(function (x) {
              return x.KABUPATEN;
            })));
          }
        }
        ...
        weatherLayer.eachLayer(function (layer) {
          ...
          for (var _i7 = 0, _Object$entries3 = Object.entries(dataTmp); _i7 < _Object$entries3.length; _i7++) {
            var _Object$entries3$_i = _slicedToArray(_Object$entries3[_i7], 2),
                _key = _Object$entries3$_i[0],
                _value = _Object$entries3$_i[1];

            if (feature.properties.feature_id == _key) {
              var customOptions;

              (function () {
                feature.properties.impacted = _value;
                var content = "";
                var count = 0;
                customOptions = {
                  'className': 'custom'
                ...
                Object.keys(feature.properties.impacted).forEach(function (key, index) {
                  id1 = count;
                  count += 1;
                  id2 = count;
                  content += '<details><summary class="question cursor-pointer select-none w-full outline-none">' + (index + 1) + ". " + key + '</summary>';
                  Object.keys(feature.properties.impacted[key]).forEach(function (key2, index2) {
                    id3 = count;
                    count += 1;
                    content += '<p class="my-auto ml-2">' + (index2 + 1) + ". " + feature.properties.impacted[key][key2] + '</p>'; // for (var i = 0; i < feature.properties.impacted[key][key2].length; i++) {
...
window.filterWeather = function (filter) {
  ...
  var checked = document.getElementById('menu-extreme-weather').checked;
  var dataUrl;
  ...
  if (checked) {
    ...
    fetch(dataUrl + '/area-' + moment(currentDate).format("YYYYMMDD") + '.json', {
      ...
      fetch(dataUrl + '/' + moment(currentDate).format("YYYYMMDD") + '.json', {
        ...
        for (var _i8 = 0, _Object$entries4 = Object.entries(feature_group); _i8 < _Object$entries4.length; _i8++) {
          ...
          for (var _i9 = 0, _Object$entries5 = Object.entries(tmp); _i9 < _Object$entries5.length; _i9++) {
        ...
        weatherLayer.eachLayer(function (layer) {
          ...
          for (var _i10 = 0, _Object$entries6 = Object.entries(dataTmp); _i10 < _Object$entries6.length; _i10++) {
            var _Object$entries6$_i = _slicedToArray(_Object$entries6[_i10], 2),
                _key2 = _Object$entries6$_i[0],
                _value2 = _Object$entries6$_i[1];

            if (feature.properties.feature_id == _key2) {
              var customOptions;

              (function () {
                feature.properties.impacted = _value2;
                var content = "";
                var count = 0;
                customOptions = {
                  'className': 'custom'
                ...
                Object.keys(feature.properties.impacted).forEach(function (key, index) {
                  ...
                  Object.keys(feature.properties.impacted[key]).forEach(function (key2, index2) {
                    id3 = count;
                    count += 1;
                    content += '<p class="my-auto ml-2" data-toggle="collapse" data-parent="#collapse' + id2 + '">' + (index2 + 1) + ". " + feature.properties.impacted[key][key2] + '</p>'; // for (var i = 0; i < feature.properties.impacted[key][key2].length; i++) {
                    //     content += '<span class="collapse" id="collapse' + id3 + '">' + (i+1)  + '. ' + feature.properties.impacted[key][key2][i] + '</span><br>'
                    // }
                  });
...
window.getHazard = function () {
  ...
  var hazards = ["CUACA_EKSTREM", "BANJIR"];
  var categories = ["", "Waspada", "Siaga", "Awas"];
  var data = {
    date: document.getElementById("date").value,
    source: document.getElementById("data-source").value
  };
  var countHazard = document.getElementsByClassName('count-hazard');

  for (var i = 0; i < countHazard.length; i++) {
    countHazard[i].innerHTML = "-";
  }

  var tableHazard = document.getElementsByClassName('table-hazard');
  ...
  fetch('getHazard?' + new URLSearchParams(data), {
    ...
    for (var i = 0; i < tableHazard.length; i++) {
      ...
      for (var value in data[hazards[i]]) {
        var maxValue = Math.max.apply(Math, data[hazards[i]][value].map(function (o) {
          return o.category;
        }));
    ...
    for (var hazard in data) {
      if (hazard != "CUACA_EKSTREM") {
        for (var _regency in data[hazard]) {
          for (var village in data[hazard][_regency]) {
            var _color2 = void 0;

            var _colorAnimation2 = void 0;

            if (data[hazard][_regency][village]["category"] == 1) {
              level = 1;
              _color2 = "rgba(124,179,66 ,1)";
              _colorAnimation2 = "green";
            } else if (data[hazard][_regency][village]["category"] == 2) {
              level = 2;
              _color2 = "rgba(251,192,45 ,1)";
              _colorAnimation2 = "yellow";
            } else if (data[hazard][_regency][village]["category"] == 3) {
              level = 3;
              _color2 = "rgba(198,40,40 ,1)";
              _colorAnimation2 = "red";
            }
            ...
            if (level == 3) {
              ...
              var hazardMarker = L.marker([data[hazard][_regency][village]['lat'], data[hazard][_regency][village]['lon']], {
                icon: myIcon_pulse
              });
              customOptions = {
                'className': 'custom'
              };
              hazardMarker.bindPopup('<div">' + '<p class="my-1 p-1 uppercase font-bold text-black" style="background-color:' + _color2 + '">Status: ' + categories[level] + '</p>' + '<p class="my-1 p-1 d-block uppercase font-bold text-white bg-blue-500" style="margin-top:5px !important;">Lokasi</p>' + '<p class="px-1 leading-5"><span class="text-blue-500">Provinsi:</span> ' + data[hazard][_regency][village]["PROVINSI"] + '</p>' + '<p class="px-1 leading-5"><span class="text-blue-500">Kabupaten/Kota:</span> ' + data[hazard][_regency][village]["KABUPATEN"] + '</p>' + '<p class="px-1 leading-5"><span class="text-blue-500">Kecamatan:</span> ' + data[hazard][_regency][village]["KECAMATAN"] + '</p>' + '<p class="px-1 leading-5"><span class="text-blue-500">Desa/Kelurahan:</span> ' + data[hazard][_regency][village]["DESA_CLEAN"] + '</p>' + '<p class="my-1 p-1 d-block uppercase font-bold text-white bg-yellow-600" style="margin-top:5px !important;">Potensi Dampak</p>' + '<p class="px-1 leading-5"><span class="text-yellow-500">Luas:</span> ' + (data[hazard][_regency][village]["count"] / 100).toLocaleString("id") + ' km<sup>2</sup></p>' + '<p class="px-1 leading-5"><span class="text-yellow-500">Populasi Terdampak:</span> ' + data[hazard][_regency][village]["sum"].toLocaleString("id") + ' jiwa</p>' + '<p class="px-1 leading-5"><span class="text-yellow-500">Fasdik Terdampak:</span> ' + data[hazard][_regency][village]["fasdik"].toLocaleString("id") + ' unit</p>' + '<p class="px-1 leading-5"><span class="text-yellow-500">Faskes Terdampak:</span> ' + data[hazard][_regency][village]["faskes"].toLocaleString("id") + ' unit</p>' + '<p class="px-1 leading-5"><span class="text-yellow-500">Fasum Terdampak:</span> ' + data[hazard][_regency][village]["fasum"].toLocaleString("id") + ' unit</p>' + '</div>', customOptions);
              hazardMarkers[hazard][categories[level]].push(hazardMarker);
            } else {
              var _hazardMarker = L.circleMarker([data[hazard][_regency][village]['lat'], data[hazard][_regency][village]['lon']], {
                radius: 4,
                fillColor: _color2,
                color: "#000",
                weight: 0.25,
              ...
              _hazardMarker.bindPopup('<div">' + '<p class="my-1 p-1 uppercase font-bold text-black" style="background-color:' + _color2 + '">Status: ' + categories[level] + '</p>' + '<p class="my-1 p-1 d-block uppercase font-bold text-white bg-blue-500" style="margin-top:5px !important;">Lokasi</p>' + '<p class="px-1 leading-5"><span class="text-blue-500">Provinsi:</span> ' + data[hazard][_regency][village]["PROVINSI"] + '</p>' + '<p class="px-1 leading-5"><span class="text-blue-500">Kabupaten/Kota:</span> ' + data[hazard][_regency][village]["KABUPATEN"] + '</p>' + '<p class="px-1 leading-5"><span class="text-blue-500">Kecamatan:</span> ' + data[hazard][_regency][village]["KECAMATAN"] + '</p>' + '<p class="px-1 leading-5"><span class="text-blue-500">Desa/Kelurahan:</span> ' + data[hazard][_regency][village]["DESA_CLEAN"] + '</p>' + '<p class="my-1 p-1 d-block uppercase font-bold text-white bg-yellow-600" style="margin-top:5px !important;">Potensi Dampak</p>' + '<p class="px-1 leading-5"><span class="text-yellow-500">Luas:</span> ' + (data[hazard][_regency][village]["count"] / 100).toLocaleString("id") + ' km<sup>2</sup></p>' + '<p class="px-1 leading-5"><span class="text-yellow-500">Populasi Terdampak:</span> ' + data[hazard][_regency][village]["sum"].toLocaleString("id") + ' jiwa</p>' + '<p class="px-1 leading-5"><span class="text-yellow-500">Fasdik Terdampak:</span> ' + data[hazard][_regency][village]["fasdik"].toLocaleString("id") + ' unit</p>' + '<p class="px-1 leading-5"><span class="text-yellow-500">Faskes Terdampak:</span> ' + data[hazard][_regency][village]["faskes"].toLocaleString("id") + ' unit</p>' + '<p class="px-1 leading-5"><span class="text-yellow-500">Fasum Terdampak:</span> ' + data[hazard][_regency][village]["fasum"].toLocaleString("id") + ' unit</p>' + '</div>', customOptions);

              hazardMarkers[hazard][categories[level]].push(_hazardMarker);
            }
          }
        }
      }
...
window.changeHazard = function (layer) {
  ...
  var level = document.getElementsByName('variable-level');
...
window.getDibi = function () {
  ...
  fetch('getDibi?' + new URLSearchParams(data), {
    ...
    regency.eachLayer(function (layer) {
      ...
      var regencyCount = data.filter(function (val) {
      ...
      dataRegency.push({
        ...
        "value": regencyCount.length,
        "color": color
      });
      var customOptions = {
        'className': 'custom'
      };
      layer.bindPopup('<div">' + '<p class="my-1 p-1 uppercase font-bold text-black" style="background-color:' + color + '">' + feature.properties.KABUPATEN + '</p>' + '<p class="px-1 leading-5"><span class="text-white">Jumlah Bencana:</span> <span class="font-bold" style="color:' + color + '">' + regencyCount.length + '</span> Kejadian</p>' + '<button class="modal-kabupaten-open text-white text-xs bg-yellow-600 px-2 py-1 ml-1 hover:bg-yellow-500" onclick="getDibiRegency(' + feature.properties.ID_KAB_CLE + ');">Informasi Detail</button>' + '</div>', customOptions);
      layer.setStyle({
        color: '#fff',
        fillColor: color,
        strokeWeight: 1,
    ...
    province.eachLayer(function (layer) {
      var feature = layer.toGeoJSON();
      var provinceCount = data.filter(function (val) {
      ...
      dataProvince.push({
        ...
        "value": provinceCount.length,
        "color": color
      });
      var customOptions = {
        'className': 'custom'
      };
      layer.bindPopup('<div">' + '<p class="my-1 p-1 uppercase font-bold text-black" style="background-color:' + color + '">' + feature.properties.PROVINSI + '</p>' + '<p class="px-1 leading-5"><span class="text-white">Jumlah Bencana:</span> <span class="font-bold" style="color:' + color + '">' + provinceCount.length + '</span> Kejadian</p>' + '<button class="modal-province-open text-white text-xs bg-yellow-600 px-2 py-1 ml-1 hover:bg-yellow-500" onclick="getDibiProvince(' + feature.properties.ID_PROV + ');">Informasi Detail</button>' + '</div>', customOptions);
      layer.setStyle({
        color: '#fff',
        fillColor: color,
        strokeWeight: 1,
    ...
    contentProvince = "";
    dataProvince.sort(function (a, b) {
      return b.value - a.value;
    });
...
window.getDibiRegency = function (id) {
  ...
  fetch('getDibiRegency?' + new URLSearchParams(data), {
    ...
    var dataColumn = [];
    var dataColumnX = [];
    dibi = groupBy(data.dibi, function (dibi) {
      return dibi.hazard;
    });
    ...
    Array.from(dibi.keys()).forEach(function (value) {
      dataTmp = [];
    ...
    Highcharts.chart('chart-regency-1', {
      ...
      tooltip: {
        ...
        pointFormat: '<tr><td style="color:{series.color};padding:0">{series.name}: </td>' + '<td style="padding:0;padding-left:2px;"><b>{point.y:.0f} Kejadian</b></td></tr>',
        footerFormat: '</table>',
        shared: true,
        useHTML: true
      },
...
window.getHotspot = function () {
  ...
  document.getElementById("loader").classList.add("show-animation");
  document.getElementById("title-date").innerHTML = moment(document.getElementById("date").value).format("DD MMMM YYYY");
  var categories = ["", "Rendah", "Sedang", "Tinggi"];
  var data = {
    date: document.getElementById("date").value,
    source: document.getElementById("data-source").value
  };
  ...
  fetch('getHotspot?' + new URLSearchParams(data), {
    ...
    for (var i = 0; i < countHazard.length; i++) {
      if (i == 0) {
        ...
        count = dataIndonesia.filter(function (el) {
          return el.level == i;
        }).length;
      }
    ...
    var provinces = dataIndonesia.reduce(function (r, a) {
    ...
    for (var _province in provinces) {
      provinceSort.push([_province, provinces[_province].length]);
    }
    ...
    var regencies = dataIndonesia.reduce(function (r, a) {
    ...
    for (var _regency2 in regencies) {
      regencySort.push([_regency2, regencies[_regency2].length]);
    }
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i]["level"] == 1) {
        level = 1;
        _color3 = "rgba(124,179,66 ,1)";
        colorAnimation = "green";
      } else if (data[i]["level"] == 2) {
        level = 2;
        _color3 = "rgba(251,192,45 ,1)";
        colorAnimation = "yellow";
      } else if (data[i]["level"] == 3) {
        level = 3;
        _color3 = "rgba(198,40,40 ,1)";
        colorAnimation = "red";
      } else {
        level = 3;
        _color3 = "rgba(198,40,40 ,1)";
      ...
      var hazardMarker = L.circleMarker([data[i]['lat'], data[i]['lon']], {
        radius: 4,
        fillColor: _color3,
        color: "#000",
        weight: 0.25,
      ...
      hazardMarker.bindPopup('<div">' + '<p class="my-1 p-1 uppercase font-bold text-black" style="background-color:' + _color3 + '">Status: ' + categories[level] + '</p>' + '<p class="my-1 p-1 d-block uppercase font-bold text-white bg-blue-500" style="margin-top:5px !important;">Lokasi</p>' + (data[i]["province"] ? '<p class="px-1 leading-5"><span class="text-blue-500">Provinsi:</span> ' + data[i]["province"] + '</p>' : "") + (data[i]["regency"] ? '<p class="px-1 leading-5"><span class="text-blue-500">Kabupaten/Kota:</span> ' + data[i]["regency"] + '</p>' : "") + (data[i]["district"] ? '<p class="px-1 leading-5"><span class="text-blue-500">Kecamatan:</span> ' + data[i]["district"] + '</p>' : "") + (data[i]["village"] ? '<p class="px-1 leading-5"><span class="text-blue-500">Desa/Kelurahan:</span> ' + data[i]["village"] + '</p>' : "") + '<p class="px-1 leading-5"><span class="text-blue-500">Koordinat:</span> ' + data[i]["lat"].toFixed(2) + (data[i]["lat"] > 0 ? " LU" : " LS") + ', ' + data[i]["lon"].toFixed(2) + ' BT</p>' + '<p class="my-1 p-1 d-block uppercase font-bold text-white bg-yellow-600" style="margin-top:5px !important;">Informasi Lahan</p>' + (data[i]["company"] ? '<p class="px-1 leading-5"><span class="text-yellow-500">Perusahaan:</span> ' + data[i]["company"] + '</p>' : "") + (data[i]["group"] ? '<p class="px-1 leading-5"><span class="text-yellow-500">Grup:</span> ' + data[i]["group"] + '</p>' : "") + (data[i]["konsesi"] ? '<p class="px-1 leading-5"><span class="text-yellow-500">Konsesi:</span> ' + data[i]["konsesi"] + '</p>' : "") + (data[i]["landcover"] ? '<p class="px-1 leading-5"><span class="text-yellow-500">Tutupan Lahan:</span> ' + data[i]["landcover"] + '</p>' : "") + '</div>', customOptions);
...
window.getFDRS = function () {
  ...
  if (document.getElementById('check-fdrs').checked) {
    tiffData = null;
    d3.request(storagePath + '/storage/fdrs/fdrs_' + moment(document.getElementById("date").value).format("YYYYMMDD") + '.tif').responseType('arraybuffer').get(function (tiffData) {
      ...
      layerData.on('click', function (e) {
        if (e.value !== null) {
          ...
          var html = "<span class=\"popupText\"> ".concat(v, "</span>");
...
window.getWeather = function () {
  ...
  tiffData = null;
  var menuLayer = document.getElementsByName('menu-layer');
  ...
  d3.request(storagePath + '/storage/weather/' + layer + '/' + layer + '-' + date + '.tif').responseType('arraybuffer').get(function (tiffData) {
    var geo = L.ScalarField.fromGeoTIFF(tiffData.response);
    layerData = L.canvasLayer.scalarField(geo, {
    ...
    layerData.on('click', function (e) {
      if (e.value !== null && action != true) {
        var v = e.value.toFixed(2);
        var html = "<span class=\"popupText\" style=\"margin-right:20px;\">".concat(layerInfo[layer][0], ": ").concat(layer == 'temp' ? (v - 273.15).toFixed(1) : v, " ").concat(layerInfo[layer][1], "</span>");
        var popup = L.popup().setLatLng(e.latlng).setContent(html).openOn(map);
      }
    });
  });
...
window.getInariskHazard = function (hazard) {
  ...
  var inariskType = document.querySelector('input[name="layer-inarisk-type"]:checked').value;
...
window.getInariskType = function (type) {
  ...
  var hazard = document.querySelector('input[name="layer-inarisk-hazard"]:checked').value;
...
window.getVolcano = function () {
  ...
  for (var _i11 = 0; _i11 < data.length; _i11++) {
    ...
    var _colorAnimation3 = void 0;

    if (parseInt(data[_i11].ga_status) == 1) {
      level = 2;
      _color4 = "#37b896";
      _colorAnimation3 = "green";
      image = data[_i11].has_vona ? "erupt1.gif" : "";
    } else if (parseInt(data[_i11].ga_status) == 2) {
      level = 3;
      _color4 = "#ffc107";
      _colorAnimation3 = "yellow";
      image = data[_i11].has_vona ? "erupt2.gif" : "";
    } else if (parseInt(data[_i11].ga_status) == 3) {
      level = 4;
      _color4 = "#dda15a";
      _colorAnimation3 = "orange";
      image = data[_i11].has_vona ? "erupt3.gif" : "";
    } else if (parseInt(data[_i11].ga_status) == 4) {
      level = 5;
      _color4 = "#e02f22";
      _colorAnimation3 = "red";
      image = data[_i11].has_vona ? "erupt4.gif" : "";
    } else {
      level = 1;
      _color4 = "#4bb2e8";
      _colorAnimation3 = "light-blue";
    }

    var randomPulse = Math.floor(Math.random() * 5 + 1);
    var myIcon_pulse = L.divIcon({
      className: 'css-icon',
      html: data[_i11].has_vona ? '<div class="marker-border marker-level' + level + ' flex justify-center pulse-animation-level' + level + '-' + randomPulse + '"><span class="align-self-center"></span><img src="' + storagePath + '/img/mountain-' + level + '.png"></div>' : '<div class="marker-level' + level + ' flex justify-center"><span class="align-center"></span></div>',
      iconSize: data[_i11].has_vona ? [50, 50] : [12.5, 12.5],
      iconAnchor: data[_i11].has_vona ? [15, 15] : [9, 5]
    });
    var point = L.marker([data[_i11].ga_lat_gapi, data[_i11].ga_lon_gapi], {
      icon: myIcon_pulse
    });
    var customOptions = {
      'className': 'custom'
    };
    point.bindPopup('<div>' + '<p class="p-1 uppercase font-bold bg-animation-' + _colorAnimation3 + '">Status: ' + categories[data[_i11].ga_status] + '</p>' + '<p class="p-1"><span class="text-blue-light font-bold">Nama:</span> G. ' + data[_i11].ga_nama_gapi + '</p>' + '<p class="p-1 bg-white bg-opacity-20"><span class="text-blue-light font-bold">Provinsi:</span> ' + data[_i11].ga_prov_gapi + '</p>' + '<p class="p-1"><span class="text-blue-light font-bold">Kabupaten/Kota:</span> ' + data[_i11].ga_kab_gapi + '</p>' + (data[_i11].activity ? '<p class="p-1 text-center font-bold bg-blue-500 uppercase">Aktivitas Terakhir</p>' + '<p class="p-1">' + data[_i11].activity.date + ' ' + data[_i11].activity.time + '</p>' + '<p class="p-1">' + data[_i11].activity.article + '</p>' : "") + (data[_i11].eruption && data[_i11].has_vona ? '<p class="p-1 text-center uppercase font-bold bg-animation-' + _colorAnimation3 + '">Erupsi Terakhir</p>' + '<p class="p-1">' + data[_i11].eruption.date + ' ' + data[_i11].eruption.time + '</p>' + '<p class="p-1">' + data[_i11].eruption.article + '</p>' : "") + // + '<p class="bg-blue-light font-bold"><span>Letusan Terakhir:</span></p>'+
    ...
    for (i = 0; i < arr.length; i++) {
      ...
      if (arr[i].name.toUpperCase().includes(val.toUpperCase())) {
        (function () {
          ...
          b.innerHTML = arr[i].name; // b.innerHTML += arr[i].name.substr(val.length);

          /*insert a input field that will hold the current array item's value:*/

          var tmp = arr[i].regency_id;
